...
async def main():
    ...
    # user_input = "Ask human to help me learn about langgraph"
    user_input_data = {
        "messages": [
            {
                "role": "user",
        ...
        if chunk.data and chunk.event != "metadata":
            if "agent" in chunk.data:
                try:
                    print(
                        "\033[93m"
                        + chunk.data["agent"]["messages"][0]["tool_calls"][0]["args"][
                            "question"
                        ]
                        + "\033[0m"
                    )
                ...
                if "__interrupt__" in chunk.data:
                    try:
                        print(
                            ...
                            + "\033[0m"
                        )  # Question coming from the graph interrupt
                    except:
                        pass
    ...
    user_response = input()
    human_response = {"data": user_response}

    tool_message = [
        {"tool_call_id": tool_call_id, "type": "tool", "content": human_response}
    ]
